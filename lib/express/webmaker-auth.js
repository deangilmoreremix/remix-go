const WebmakerAuth = require('webmaker-auth');
const describeWhiteLabel = require('./describe-whitelabel');

const {
  loginServer: {
    url: loginURL,
    authUrl,
    urlWithAuth: authLoginURL,
    username,
    password,
  },
  useWhiteLabels,
  prefixes,
  appHostname: loginHost,
  secret: secretKey,
  forceSsl: forceSSL,
  cookieDomain: domain,
} = require('../../config/config');

const whitelabelOptions = {
  prefix: prefixes.app,
  apiPrefix: prefixes.api,
  loginPrefix: prefixes.login,
  editorPrefix: prefixes.editor,
  projectsPrefix: prefixes.projects,
  cdnPrefix: prefixes.cdn,
  loginUsername: username,
  loginPassword: password,
};

const webmakerAuth = new WebmakerAuth({
  loginURL, loginHost, authLoginURL, secretKey, forceSSL, domain,
});

module.exports = (app) => {
  if (useWhiteLabels) {
    app.use(webmakerAuth.extractWhitelabel(whitelabelOptions));
    app.use(describeWhiteLabel);
  }
  app.use(webmakerAuth.cookieParser());
  app.use(webmakerAuth.cookieSession());

  // TODO: implement logout call as POST because it's not idempotent call
  app.get('/logout', (req, res) => {
    req.session.email = null;
    req.session.user = null;
    req.session.refreshAfter = null;
    return res.redirect('/');
  });
  app.get('/account', (req, res) => res.redirect(`${authUrl}/account`));
};
