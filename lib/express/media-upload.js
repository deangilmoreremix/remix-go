...
module.exports.mediaUpload = (req, res) => {
  function uploadFromFile(req, res) {
    ...
    return fs
      ...
      .then(sourceMedia => (isVideoSource ? s3.mediaClient : s3).putBuffer(sourceMedia, s3Url, {
        ...
        if (isVideoSource) {
          ...
          if (req.query.video_preview) {
            const previewUrl = `${s3Path}.webm`;
            return childProcess
              .execAsync(`ffmpeg -i ${media.path} -c:v libvpx -crf 20 -ss 0 -t 3 ${media.path}.webm`)
              .then(() => fs.readFileAsync(`${media.path}.webm`))
              .then(previewMedia => Bb.fromCallback((callback) => {
                s3.putBuffer(previewMedia, previewUrl, {
                  'x-amz-acl': 'public-read',
              ...
              .then((response) => {
                ...
                if (response.statusCode !== 200) {
                  return res.json({ url: `${config.s3.cdn}${s3Url}` });
                }
                return res.json({
                  url: `${config.s3.cdn}${s3Url}`,
                  preview: `${config.s3.cdn}${previewUrl}`,
                });
              })
              .catch(() => res.json({ url: `${config.s3.cdn}${s3Url}` }));
          } else {
            cleanUpTempFiles(parseTempFiles(req));
            return res.json({ url: `${config.s3.cdn}${s3Url}` });
          }
