...
class LinkedinSocialProvider extends AbstractSocialProvider {
  ...
  static CLIENT_ID = '77kh3rpp8kfpgs';
  static ACCESS_SCOPES = 'r_basicprofile r_emailaddress w_share';
  static OAUTH_CALLBACK = 'http://dev-api.videoremix.io/api/makes/social-proxy/linkedin/oauth2-callback';
  static PROFILE_FIELDS = 'picture-urls::(original),first-name,last-name,email-address,formatted-name';
  ...
  init() {
    this.api = requestCreator(
      'dev-api.vidcloud.io/api/makes/social-proxy/linkedin',
        ...
        const loginCheckInterval = setInterval(() => {
          if (!linkedinLoginWindow || linkedinLoginWindow.closed) {
            ...
            if (this._retrieveAccessToken()) {
              ...
              reject(new Error({ message: 'Auth canceled.' }));
            }
          }
        }, 100);
      } else {
