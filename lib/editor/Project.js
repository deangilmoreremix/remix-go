...
class Project {
  ...
  checkForm() {
    const tokens = this.getOverriddenTokens();
    this.setWarning(tokens.length > 0 ? 'Some form field tokens also appear in personalized '
      + 'elements before the form on the timeline. This could cause the matching form fields to to '
      + 'pre-populate during playback. The matching duplicated tokens are:\n' : null, tokens)();
  }
  ...
  getOverriddenTokens() {
    ...
    this.projectData.media.forEach(({ tracks: projectTracks }) => {
  ...
  @action
  update(element, options, updateMain) {
    this.modified = true;
    console.log(`updating element with options ${JSON.stringify(options)}`);
    this.projectData.media.forEach((media) => {
      media.tracks.forEach((track) => {
        track.trackEvents.forEach((trackEvent) => {
