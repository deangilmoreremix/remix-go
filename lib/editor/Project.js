...
class Project {
  ...
  set cta(value) {
    ...
    this.projectData.media.forEach((media) => {
      media.tracks.forEach((track) => {
        track.trackEvents.forEach((trackEvent) => {
          if (trackEvent.popcornOptions.start === ctaCheckpoint) {
            const options = _.omit(
              ctaElements.filter(item => item.type === trackEvent.type)[0].popcornOptions,
              ['start', 'end'],
            );
            if (this._engine) {
              const popcornEvent = this._engine.data.trackRefs[trackEvent.popcornOptions.id];
              /* eslint-disable no-underscore-dangle */
              popcornEvent._natives._update.call(this, popcornEvent, options);
            }
