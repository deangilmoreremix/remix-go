...
class Project {
  ...
  set loop(value) {
    if (value) {
      ...
      if (this.stopOnCta) {
        ...
        loopEndTime = pauseElement.popcornOptions.start + 0.1;
      } else {
        loopEndTime = this.projectData.media[0].duration - 0.2;
      }
      this.add({
        type: 'loopPlugin',
        popcornOptions: {
  ...
  constructor(make = {}) {
    ...
    this.elements.forEach(({ type, popcornOptions: { start } }) => {
      ...
      if (this._checkpoints.indexOf(start) === -1 &&
        Project.EditableElementTypes.indexOf(type) !== -1) {
        this._checkpoints.push(start);
      }
    });
    this._checkpoints = this._checkpoints.sort((a, b) => a - b);
  }
