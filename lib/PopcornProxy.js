...
module.exports = {
  ...
  videoResizer: (element, options) => () => {
    const { padding = 10, baseFontSize = 14, baseVideoWidth = 560 } = options;
    let { ratio = { width: 16, height: 9 } } = options;
    ...
    wrapper.style.padding = (maxWidth - (maxHeight * ratio)) / 2 > 0
      ? `${padding}px ${((maxWidth - (maxHeight * ratio)) / 2) + padding}px`
      : `${((maxHeight - (maxWidth / ratio)) / 2) + padding}px ${padding}px`;
    wrapper.style.fontSize = `${baseFontSize * (wrapper.offsetWidth / baseVideoWidth)}px`;
  },
};
