...
module.exports = {
  ...
  videoResizer: (element, padding = 10, baseFontSize = 14, baseVideoWidth = 560) => () => {
    ...
    const sideIndent = (maxWidth - ((maxHeight * 16) / 9)) / 2;
debugger
    wrapper.style.padding = sideIndent > 0
      ? `${padding}px ${sideIndent}px`
      : `${((maxHeight - ((maxWidth * 9) / 16)) / 2) + padding}px ${padding}px`;
    video.style.fontSize = `${baseFontSize * (519 / baseVideoWidth)}px`;
  },
};
