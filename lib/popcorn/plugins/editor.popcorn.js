...
export default class PopcornEditor extends Component {
  static propTypes = {
    element: PropTypes.any, // TODO: define prop type
  ...
  buildInput() {
    const resultInput = document.querySelector(".font-input");
    this.loadStyle(resultInput.fontFamily, resultInput);
    const googleFonts = this.fonts.sort();
    googleFonts.forEach(function (font) {
      const option = document.createElement('option');
      option.value = option.innerHTML = font;
      this.loadStyle(font, option);
      if (resultInput.fontFamily && resultInput.fontFamily === option.value) {
        option.setAttribute('selected', true);
      }
      resultInput.appendChild(option);
    }, this);
    resultInput.addEventListener('change', function (e) {
      resultInput.style.fontFamily = e.target.value;
    });
  }
}
