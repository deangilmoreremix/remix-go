...
(function (Popcorn) {
  ...
  function unwrapTokens(string) {
    const tokenRegexPattern = `${OPEN_PERSONALIZATION_TAG}(<span class="(token-default|token-uppercase)" data-parameter="([^="<>]*)"></span>)?([^{}="<>]*)${CLOSE_PERSONALIZATION_TAG}`;
  ...
  function createImageDiv(element, imageUrl) {
    ...
    linkSpan.addEventListener('keyup', catchCaretCharacterOffsetWithin(element, 'src'));
    linkSpan.addEventListener('input', catchCaretCharacterOffsetWithin(element, 'src'));
    linkSpan.setAttribute('contenteditable', '');
    linkSpan.addEventListener('input', (event) => {
      element.src = unwrapTokens(event.target.innerHTML);
      element._context.emit('elementUpdated', {
        type: 'personalizedImage',
        element,
        options: {
          src: unwrapTokens(event.target.innerHTML),
        },
      });

      const caretPositionShifting = element.caretOffsets.src -
        prettyPrintTokens(
          event.target.innerText.substr(0, element.caretOffsets.src),
        ).length;
      if (caretPositionShifting > 0) {
        event.target.innerHTML = wrapTokens(element.src);
        element.caretOffsets.src -= caretPositionShifting;
        const selection = window.getSelection();
        if (selection) {
          let offset = element.caretOffsets.src;
          let nodeIndex;
          for (nodeIndex = 0;
    ...
    linkSpan.innerHTML = wrapTokens(element.src);
