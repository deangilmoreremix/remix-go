...
define(['../../util/facebook-data-provider'], function (fbDataProvider) {
  var Popcorn = window.Popcorn;
  var urlRegex = /[\w-]+(\.[\w-]+)+([\w.,@?^=%&amp;:\/~+#-]*[\w@?^=%&amp;\/~+#-])?/;
  var defaultHref = 'https://www.facebook.com/facebook';
  var defaultPostUrl = 'https://www.facebook.com/20531316728/posts/10154009990506729';
  var defaultEmbedCommentUrl = 'https://www.facebook.com/zuck/posts/10102577175875681?comment_id=1193531464007751&reply_comment_id=654912701278942';
  var vrPublisherAppId = window.facebookAppId || '1728968890675795';
  var FB_PLUGINS = {
    'fb-like': {
      title: 'Like',
      'show-faces': true,
  ...
  function setAttributes(options) {
    var fbAttrs = FB_PLUGINS[options.type];
    var neededFbAttrs = Object.keys(fbAttrs);
    var defaultHrefs = [defaultHref, defaultEmbedCommentUrl, defaultPostUrl];
    var facebookDiv = options._facebookDiv;
    var attributes = facebookDiv.attributes;
    var attributeNames = [];
    var oldHref = facebookDiv.getAttribute('data-href');
    var oldWidth = facebookDiv.getAttribute('data-width');
    var oldHeight = facebookDiv.getAttribute('data-height');
    var oldIncludeParent = facebookDiv.getAttribute('data-include-parent') || '';

    for (var i = 0; attributes, i < attributes.length; i++){
      attributeNames.push(attributes[i].nodeName.replace('data-', ''));
    }

    Popcorn.forEach(fbAttrs, function (value, key) {
      var fbAttrs = FB_PLUGINS[options.type];
      var facebookDiv = options._facebookDiv;
      ...
      if (key === 'width') {
        if (window.Butter) {
          ...
          actualWidth = options[key] ? options[key] * target.clientWidth / 100 : options.editorWidth || fbAttrs.minWidth;
        }
      ...
      if (key === 'height' && options.type === 'fb-page') {
        ...
        if (window.Butter) {
          ...
          actualHeight = options[key] ? options[key] * target.clientHeight / 100 : options.editorHeight ||  fbAttrs.minHeight;
        }
    ...
    var newWidth = options.editorWidth;
    var newHeight = options.editorHeight;
    var hrefChanged = (oldHref !== options.href);
    var typeChanged = !options._facebookDiv.classList.contains(options.type);
    var widthChanged = newWidth && (parseInt(oldWidth) !== newWidth);
    var heightChanged;
    var includeParentChanged = (oldIncludeParent !== (options['include-parent'] && options['include-parent'].toString() || ''));

    if (!typeChanged && (options.type === 'fb-page')) {
      heightChanged = newHeight && (parseInt(oldHeight) !== newHeight);
    }
    var somethingChanged = hrefChanged || typeChanged || widthChanged || heightChanged || includeParentChanged;

    if (typeChanged) {
      facebookDiv.setAttribute('class', options.type);

      // remove attributes we do not need
      Popcorn.forEach(attributeNames, function (attribute) {
    ...
    var draggableHandle = options._container.querySelector('.ui-draggable-handle');
    if (draggableHandle) {
