...
(function( Butter, VariableDescriptorFactory ) {
  ...
  Butter.Editor.register( "text", "load!{{baseDir}}plugins/text-editor.html",
    ...
    function setup( trackEvent ) {
      ...
      function attachHandlers() {
        ...
        function checkboxCallback( trackEvent, updateOptions, prop ) {
          if ( "background shadow stroke".match( prop ) ) {
          ...
          if ( pluginOptions.hasOwnProperty( key ) ) {
            ...
            if ( option.elementType === "select" ) {
              ...
              if ( option.element.type === "checkbox" ) {
                ...
                pickers.stroke = option.element;
                // set initial state
                if ( !_popcornOptions.stroke ) {
                  option.element.classList.add( "butter-disabled" );
                  option.element.onclick = _falseClick;
                }
                _this.attachColorChangeHandler( option.element, option.trackEvent, key, colorCallback );
              ...
              option.element.parentElement.appendChild(VariableDescriptorFactory.create({ target: option.element, callback: _this.updateTrackEventSafe, trackEvent: option.trackEvent, showTooltip: true }));
              _this.attachInputChangeHandler( option.element, option.trackEvent, key, _this.updateTrackEventSafe );
            }
          }
        }
    ...
    this.resizable( trackEvent, _container, target, {
      minWidth: 1,
      handlePositions: trackEvent.popcornOptions.fontDecorations.responsive ? "n,s,e,w" : "e,w",
      end: function() {
        if ( trackEvent.popcornOptions.position !== "custom" ) {
          trackEvent.update({
            position: "custom"
    ...
    this.rotatable( trackEvent, _container, target );
