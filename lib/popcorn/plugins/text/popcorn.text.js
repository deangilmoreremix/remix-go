...
(function (Popcorn, jQuery) {
  ...
  function catchCaretCharacterOffsetWithin({ target: element }) {
    let caretOffset = 0;
  ...
  Popcorn.plugin('text', {
    ...
    _setup(options) {
      ...
      if (linkUrl) {
        ...
        link.addEventListener('click', catchCaretCharacterOffsetWithin);
        ...
        innerSpan.addEventListener('click', catchCaretCharacterOffsetWithin);
    ...
    _update(trackEvent, options) {
      function runTextfill() {
        ...
        jQuery(trackEvent._container.firstChild).textfill(resizeOptions);
      }
