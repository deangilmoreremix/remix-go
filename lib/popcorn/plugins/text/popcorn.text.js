...
(function (Popcorn, jQuery) {
  ...
  function unwrapTokens(string) {
    const smartTokenRegex = new RegExp(`${OPEN_PERSONALIZATION_TAG}(<span class="(.*)" data-parameter="(.*)"></span>)?([^{}]*)${CLOSE_PERSONALIZATION_TAG}`, 'gm');
    return string.replace(smartTokenRegex, (match) => {
      const [, , helper, params, token] = smartTokenRegex.exec(match);
      if (helper) {
        const [tokenHelper] = Object
          .values(TOKEN_HELPER_CLASSES)
