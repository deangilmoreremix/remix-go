...
(function (Popcorn, jQuery) {
  ...
  const OPEN_PERSONALIZATION_TAG = '<span class="personalized-token" contenteditable="false"><span class="token-uppercase"></span>';
  ...
  function wrapTokens(string) {
    return string
      .replace(/{{/gm, OPEN_PERSONALIZATION_TAG)
      .replace(/}}/gm, CLOSE_PERSONALIZATION_TAG);
  }

  function unwrapTokens(string) {
    return string
      .replace(new RegExp(OPEN_PERSONALIZATION_TAG, 'gm'), '{{')
      .replace(new RegExp(CLOSE_PERSONALIZATION_TAG, 'gm'), '}}');
  }
