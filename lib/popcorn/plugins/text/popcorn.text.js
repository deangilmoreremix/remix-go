...
(function (Popcorn, jQuery) {
  ...
  function wrapTokens(string) {
    ...
    return string.replace(tokenRegex, (match) => {
      ...
      if (match.split(' ').length > 1) {
        ...
        result += `<span class="${TOKEN_HELPER_CLASSES[helper]}"></span>`;
        result += tokenName;
        result += params.map(item => `<span class="parameter">${item}</span>`).join('');
      } else {
        result += match;
      }
  ...
  function unwrapTokens(string) {
    const smartTokenRegex = new RegExp(`${OPEN_PERSONALIZATION_TAG}[^{}]*${CLOSE_PERSONALIZATION_TAG}`, 'gm');
    return string.replace(smartTokenRegex, (match) => {
      match = match
        .replace(OPEN_PERSONALIZATION_TAG, '{{')
        .replace(CLOSE_PERSONALIZATION_TAG, '}}');
      Object.values(TOKEN_HELPER_CLASSES).forEach(([key, value]) => {
        match = match.replace(`<span class="${value}"></span>`, `${key} `);
      });
      match = match.replace(new RegExp('<span className="parameter">.*</span>', 'gm'), (inmatch) => {
        return inmatch
          .replace('<span className="parameter">', '')
          .replace('</span>', '');
      });
      return match;
    });
