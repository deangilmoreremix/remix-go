...
(function (Popcorn, jQuery) {
  ...
  function catchCaretCharacterOffsetWithin(options) {
    return ({ target: element }) => {
      ...
      options.caretOffset = caretOffset;
    };
  }
  ...
  Popcorn.plugin('text', {
    ...
    _setup(options) {
      ...
      if (linkUrl) {
        ...
        link.addEventListener('click', catchCaretCharacterOffsetWithin(options));
        options._contentContainer = link;
        ...
        innerSpan.addEventListener('click', catchCaretCharacterOffsetWithin(options));
    ...
    _update(trackEvent, options) {
      ...
      if (trackEvent.linkUrl) {
        ...
        link.href = trackEvent.linkUrl;
        link.target = trackEvent.linkTarget;
        link.innerHTML = trackEvent.text;

        link.addEventListener('click', () => {
          trackEvent._context.media.pause();
        });
        link.addEventListener('click', catchCaretCharacterOffsetWithin(trackEvent));
        trackEvent._contentContainer = link;

        link.style.color = trackEvent._container.firstChild.style.color;
        link.style.fontFamily = trackEvent.fontFamily;

        innerSpan.appendChild(link);
      } else {
        innerSpan.innerHTML = trackEvent.text;
        innerSpan.addEventListener('click', catchCaretCharacterOffsetWithin(trackEvent));
      ...
      if (trackEvent.position === 'top') {
        trackEvent._container.style.left = `${trackEvent.padding}%`;
        trackEvent._container.style.width = `${trackEvent.width}%`;
        trackEvent._container.style.top = `${trackEvent.padding}%`;
      } else if (trackEvent.position === 'bottom') {
        trackEvent._container.style.left = `${trackEvent.padding}%`;
        trackEvent._container.style.width = `${trackEvent.width}%`;
        trackEvent._container.style.top = `${100 - trackEvent.padding - trackEvent.fontSize}%`;
      ...
      if (trackEvent.fontDecorations.responsive) {
        ...
        if (trackEvent.height) {
          trackEvent._container.style.height = `${options.height}%`;
