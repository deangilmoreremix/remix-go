...
(function (Popcorn) {
  ...
  function createText(canvas, options) {
    ...
    var vH = v.offsetHeight, vW = v.offsetWidth;
    var text = options.text;
    var bgcolor = DEFAULT_BACKGROUND;
    var fontFamily = options.fontFamily;

    var fontDecorations = options.fontDecorations;
    var trans = options.backgroundTransparent;
    ...
    var tempcanvaswidth = (ctx.canvas.width - textDimensions.width) / 2;
    var heightAdjust = 0.75;
    if (fontFamily === "Fjalla One" || fontFamily === "Oswald") {
      heightAdjust = heightAdjust + 0.05;
    }
    if ((fontFamily === "Bowlby One SC") && (text.length > 3)) {
      heightAdjust = heightAdjust - 0.10;
    }
    if (text.length <= 3) {
      heightAdjust = heightAdjust + 0.10;
    }
    ...
    var txtHeight = (ctx.canvas.height * heightAdjust);
  ...
  Popcorn.plugin("seethroughtext", {
    ...
    _setup: function (options) {
      ...
      var innerDivCanvas = document.createElement("canvas");
