  ...
  Popcorn.plugin('image', {
    ...
    _setup(options) {
      ...
      _container.addEventListener('click', (event) => {
        ...
        const linkUrlSpan = document.querySelector('link-url-span');
        const { currentUser } = options._context;
        if (!options.linkSrc && !linkUrlSpan.innerHTML) {
          linkUrlSpan.innerHTML = wrapTokens(callToActionFeature[currentUser.features.clickToPhoneCall.state]);
        }
    ...
    _update(trackEvent, options) {
      ...
      if ((options.src !== undefined && options.src !== trackEvent.src)
        ...
        imageDiv.style.backgroundImage = `url( "${trackEvent.src}" )`;
