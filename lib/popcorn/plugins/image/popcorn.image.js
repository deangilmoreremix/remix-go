    ...
    _setup(options) {
      ...
      function setupImageDiv() {
        ...
        linkUrlSpan.addEventListener('input', (event) => {
          options.linkSrc = event.target.outerText || event.target.innerText;
        ...
        linkUrlSpan.addEventListener('paste', (e) => {
          ...
          document.execCommand(
            'insertHTML', false, (e.clipboardData || window.clipboardData).getData('text/plain'),
          );
        });
        if (options.linkSrc) {
          linkUrlSpan.innerText = options.linkSrc;
