...
(function (Popcorn) {
  ...
  Popcorn.plugin('image', {
    ...
    _setup(options) {
      ...
      _image.style.borderRadius = `${options.cornerRadius}%`;
      if (option.background) {
        _image.style.backrgound = options.backgroundColor;
      }
      if (_target) {
    ...
    _update(trackEvent, options) {
      ...
      if (trackEvent.innerHeight) {
        trackEvent.image.style.height = `${validateDimension(trackEvent.innerHeight, '0')}%`;
      }
      if (trackEvent.innerWidth) {
        trackEvent.image.style.width = `${validateDimension(trackEvent.innerWidth, '0')}%`;
      }
      trackEvent.image.style.borderRadius = `${options.cornerRadius}%`;
      if (trackEvent.background) {
        trackEvent.image.style.backrgound = trackEvent.backgroundColor;
      }
    },
