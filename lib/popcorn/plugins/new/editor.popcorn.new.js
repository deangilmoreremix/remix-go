...
export default class NewElementBar extends PopcornEditor {
  ...
  render() {
    const { project, defaultImage, features, store, store: { currentUser } } = this.props;
    return (
      <div className="popcorn-editor new-bar">
        ...
        { (store.isAdmin() ||
          (
            currentUser.features[features.leadgen] &&
            currentUser.features[features.leadgen].state === 'enabled')
        )
          && (
          <button
            className="action-button"
