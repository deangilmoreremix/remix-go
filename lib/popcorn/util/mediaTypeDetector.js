...
class MediaTypeDetector {
  ...
  async getMetadata(baseUrl, contentType) {
    ...
    if (type === 'YouTube') {
      ...
      const parsedUri = this.uri.parse(baseUrl);
      const splitUriDirectory = parsedUri.directory.split('/');
      let id = splitUriDirectory[splitUriDirectory.length - 1];
      let xhrURL = `https://vimeo.com/api/v2/video/${id}.json`;
      if (Number.isNaN(id) === true) {
        id = splitUriDirectory[splitUriDirectory.length - 2];
        xhrURL = `https://vimeo.com/api/oembed.json?url=https://vimeo.com/${id}`;
      }
      let resp = await handleFetchResponse(await fetch(xhrURL, {
        Accept: 'application/json',
        'Content-Type': 'application/json',
      }));

      const source = `http://vimeo.com/${id}`;
      if (!resp) {
