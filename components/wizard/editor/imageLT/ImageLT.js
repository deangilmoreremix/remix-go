...
import ImageLTItem from './ImageLTItem';
import EmbeddedPlayback from '../../../common/EmbeddedPlayback';
@inject('api')
@observer
export default class ImageLT extends Component {
  static propTypes = {
  ...
  onSearch = async (query) => {
    ...
    const newElements = await api.imageLT(0, query);
    this.setState({
  ...
  loadMore = async () => {
    ...
    const newElements = await api.imageLT(elements.length, query);
  ...
  render() {
    ...
    // if(this.state.elements.length && this.state.activeItem == "") {
    //   this.setState({
    //     activeItem:this.state.elements[0]
    //   })
    // }
    return (
      <Fragment>
